...
module.exports = {
  ...
  add: async (values) => {
    ...
    return Wishlist.updateRelations({ _id: entry.id, values: relations });
  },
  ...
  edit: async (params, values) => {
    ...
    const entry = await Wishlist.update(params, data, { multi: true });

    // Update relational data and return the entry.
    return Wishlist.updateRelations(Object.assign(params, { values: relations }));
  },
  ...
  remove: async params => {
    ...
    await Promise.all(
      Wishlist.associations.map(async association => {
  ...
  search: async (params) => {
    ...
    const $or = Object.keys(Wishlist.attributes).reduce((acc, curr) => {
      switch (Wishlist.attributes[curr].type) {
        case 'integer':
        case 'float':
        case 'decimal':
